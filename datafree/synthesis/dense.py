...
from utils_fl import setup_seed
...
class DENSESynthesizer(BaseSynthesis):
    def __init__(self, teacher, mdl_list, student, generator, nz, num_classes, img_size, save_dir, iterations=1,
                 ...
                 autocast=None, use_fp16=False, distributed=False, args=None, seed=0):
    ...
    def synthesize(self, cur_ep=None, debias=False, debias_start=50):
        ###########
        # .eval()
        ###########
        ...
        for it in range(self.iterations):
            ...
            if (debias==True):
                with torch.no_grad():
                    white = torch.ones((1, 3, 32, 32)).to(self.device)
                    bias = self.teacher(white)
                    if (cur_ep >= debias_start):
                        print("-----debias Gen-----")
                        t_out = t_out - bias.detach()
