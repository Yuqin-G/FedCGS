...
class ProjandDeci_norm(nn.Module):
    def __init__(self, in_d, out_d, num_classes):
        super(ProjandDeci, self).__init__()
        self.fc1 = nn.Linear(in_d, out_d)
        self.fc2 = nn.Linear(out_d, num_classes)
...
def resnet18_fc(num_classes=10):
    model = ResNet(BasicBlock, [2, 2, 2, 2], num_classes)
    for param in list(model.parameters())[:-2]:  # 除最后一层外的所有参数
        param.requires_grad = False
    # num_ftrs = model.fc.in_features
    # model.fc = nn.Linear(num_ftrs, num_classes) 
    return model

def resnet18_fc2(num_classes=10):
    model = ResNet(BasicBlock, [2, 2, 2, 2], num_classes)
    model.linear = ProjandDeci(512, 256, num_classes) 
    for param in list(model.parameters())[:-4]:  # 除最后一层外的所有参数
        param.requires_grad = False
    # num_ftrs = model.fc.in_features
    # model.fc = nn.Linear(num_ftrs, num_classes) 
    return model

def resnet18_fc3(num_classes=10):
    model = ResNet(BasicBlock, [2, 2, 2, 2],num_classes)
    model.fc = ProjandDeci(512, 256, num_classes) 
    for param in list(model.parameters())[:-4]:  # 除最后一层外的所有参数
        param.requires_grad = False
    # num_ftrs = model.fc.in_features
    # model.fc = nn.Linear(num_ftrs, num_classes) 
